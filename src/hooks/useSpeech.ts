...
export function useSpeech(language: string = "en"): UseSpeechResult {
  ...
  // Check speech support and test actual functionality
  useEffect(() => {
    const hasRecognition =
      "SpeechRecognition" in window || "webkitSpeechRecognition" in window;
    const hasSynthesis = "speechSynthesis" in window;

    // Initially assume it's supported if APIs exist
  ...
  // Initialize speech recognition with network error detection
  useEffect(() => {
    if (!speechSupported || networkErrorDetected) return;
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      if (event.error === "network") {
        // Permanently disable speech recognition for this session
        setNetworkErrorDetected(true);
        setSpeechSupported(false);
        setError(
          "Speech recognition requires internet connection and may not work in all browsers. Please use the text input instead."
        );
      } else {
  ...
  }, [speechSupported, networkErrorDetected]);

  // Start listening with network error check
  const startListening = () => {
    if (networkErrorDetected) {
      setError(
        "Speech recognition is not available on this device/browser. Please type your message instead."
      );
      return;
    }
